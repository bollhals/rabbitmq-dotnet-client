...
namespace RabbitMQ.Client.Framing
{
    ...
    public struct MessageProperties : IBasicProperties, ISpanWriteable
    {
        ...
        public void ClearContentType() => _contentType = default;

        public void ClearContentEncoding() => _contentEncoding = default;

        public void ClearHeaders() => _headers = default;

        public void ClearDeliveryMode() => _deliveryMode = default;

        public void ClearPriority() => _priority = default;

        public void ClearCorrelationId() => _correlationId = default;

        public void ClearReplyTo() => _replyTo = default;

        public void ClearExpiration() => _expiration = default;

        public void ClearMessageId() => _messageId = default;

        public void ClearTimestamp() => _timestamp = default;

        public void ClearType() => _type = default;

        public void ClearUserId() => _userId = default;

        public void ClearAppId() => _appId = default;

        public void ClearClusterId() => _clusterId = default;

        public readonly bool IsContentTypePresent() => _contentType != default;

        public readonly bool IsContentEncodingPresent() => _contentEncoding != default;

        public readonly bool IsHeadersPresent() => _headers != default;

        public readonly bool IsDeliveryModePresent() => _deliveryMode != default;

        public readonly bool IsPriorityPresent() => _priority != default;

        public readonly bool IsCorrelationIdPresent() => _correlationId != default;

        public readonly bool IsReplyToPresent() => _replyTo != default;

        public readonly bool IsExpirationPresent() => _expiration != default;

        public readonly bool IsMessageIdPresent() => _messageId != default;

        public readonly bool IsTimestampPresent() => _timestamp != default;

        public readonly bool IsTypePresent() => _type != default;

        public readonly bool IsUserIdPresent() => _userId != default;

        public readonly bool IsAppIdPresent() => _appId != default;

        public readonly bool IsClusterIdPresent() => _clusterId != default;

        public MessageProperties(ReadOnlySpan<byte> span)
        {
            ...
            if (contentType_present) { offset += WireFormatting.ReadShortstr(span.Slice(offset), out _contentType); } else { _contentType = default; }
            if (contentEncoding_present) { offset += WireFormatting.ReadShortstr(span.Slice(offset), out _contentEncoding); } else { _contentEncoding = default; }
            if (headers_present) { offset += WireFormatting.ReadDictionary(span.Slice(offset), out var tmpDirectory); _headers = tmpDirectory; } else { _headers = default; }
            if (deliveryMode_present) { _deliveryMode = span[offset++]; } else { _deliveryMode = default; }
            if (priority_present) { _priority = span[offset++]; } else { _priority = default; }
            if (correlationId_present) { offset += WireFormatting.ReadShortstr(span.Slice(offset), out _correlationId); } else { _correlationId = default; }
            if (replyTo_present) { offset += WireFormatting.ReadShortstr(span.Slice(offset), out _replyTo); } else { _replyTo = default; }
            if (expiration_present) { offset += WireFormatting.ReadShortstr(span.Slice(offset), out _expiration); } else { _expiration = default; }
            if (messageId_present) { offset += WireFormatting.ReadShortstr(span.Slice(offset), out _messageId); } else { _messageId = default; }
            if (timestamp_present) { offset += WireFormatting.ReadTimestamp(span.Slice(offset), out _timestamp); } else { _timestamp = default; }
            if (type_present) { offset += WireFormatting.ReadShortstr(span.Slice(offset), out _type); } else { _type = default; }
            if (userId_present) { offset += WireFormatting.ReadShortstr(span.Slice(offset), out _userId); } else { _userId = default; }
            if (appId_present) { offset += WireFormatting.ReadShortstr(span.Slice(offset), out _appId); } else { _appId = default; }
            if (clusterId_present) { WireFormatting.ReadShortstr(span.Slice(offset), out _clusterId); } else { _clusterId = default; }
        }

        public ushort ProtocolClassId => 60;
        public string ProtocolClassName => "basic";

        int ISpanWriteable.WritePropertiesTo(Span<byte> span)
        {
        ...
        internal readonly int GetRequiredPayloadBufferSize()
        {
